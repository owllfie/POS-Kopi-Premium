...
<div class="space-y-6" x-data="categoryManager()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex border-b border-coffee-latte">
            <a href="{{ route('kategori.index', ['tab' => 'active']) }}" class="px-6 py-2.5 font-bold text-xs border-b-2 transition {{ $tab === 'active' ? 'border-coffee-dark text-coffee-dark' : 'border-transparent text-coffee-light hover:text-coffee-dark' }}">
                Kategori Aktif
            </a>
            <a href="{{ route('kategori.index', ['tab' => 'trash']) }}" class="px-6 py-2.5 font-bold text-xs border-b-2 transition {{ $tab === 'trash' ? 'border-coffee-dark text-coffee-dark' : 'border-transparent text-coffee-light hover:text-coffee-dark' }}">
                Tong Sampah (Trash)
            </a>
            <a href="{{ route('kategori.index', ['tab' => 'history']) }}" class="px-6 py-2.5 font-bold text-xs border-b-2 transition {{ $tab === 'history' ? 'border-coffee-dark text-coffee-dark' : 'border-transparent text-coffee-light hover:text-coffee-dark' }}">
                Riwayat Perubahan
            </a>
        </div>
                ...
                <table class="w-full text-left border-collapse text-sm">
                    <thead>
                        <tr class="border-b border-coffee-latte text-xs font-bold text-coffee-light uppercase tracking-wider">
                            ...
                            <th class="pb-3">Jumlah Item Menu</th>
                    ...
                    <tbody class="divide-y divide-coffee-latte font-medium text-coffee-dark">
                        @forelse($categories as $c)
                            <tr>
                                <td class="py-3.5 font-bold text-coffee-dark">{{ $c->kategori }}</td>
                                <td class="py-3.5 text-xs text-coffee-light">
                                    <span class="bg-coffee-latte px-2 py-0.5 rounded font-bold text-[10px]">{{ $c->menus_count ?? 0 }} Items</span>
                                </td>
                                <td class="py-3.5 text-right">
                                    <div class="flex justify-end gap-1.5">
                                        @if($tab === 'active')
                                            <button @click="openEdit({{ json_encode($c) }})" class="p-1.5 rounded-lg hover:bg-amber-50 text-coffee-light hover:text-coffee-dark transition cursor-pointer" title="Edit Kategori">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            </button>
                                            <form action="{{ route('kategori.delete', $c->id_kategori) }}" method="POST" onsubmit="return confirm('Hapus kategori ini beserta menunya?')">
                                                @csrf
                                                <button type="submit" class="p-1.5 rounded-lg hover:bg-rose-50 text-rose-500 hover:text-rose-700 transition cursor-pointer" title="Hapus Kategori">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                </button>
                                            </form>
                                        @else
                                            <form action="{{ route('kategori.restore', $c->id_kategori) }}" method="POST">
                                                @csrf
                                            ...
                                            <form action="{{ route('kategori.forceDelete', $c->id_kategori) }}" method="POST" onsubmit="return confirm('Hapus PERMANEN kategori ini? Menu terkait juga akan terdampak.')">
                                                @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-8 text-center text-coffee-light font-medium">Tidak ada data kategori.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        >
            ...
            >
                <div class="flex items-center justify-between border-b border-coffee-latte pb-3">
                    <h3 class="font-extrabold text-coffee-dark">Tambah Kategori Menu</h3>
                ...
                <form action="{{ route('kategori.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="kategori" class="block text-xs font-bold text-coffee-medium uppercase tracking-wider mb-2">Nama Kategori</label>
                        <input type="text" name="kategori" id="kategori" required class="w-full px-4 py-2.5 rounded-xl border border-coffee-latte text-xs font-bold text-coffee-dark focus:outline-none focus:ring-2 focus:ring-coffee-light/50 bg-white" placeholder="Contoh: Snack, Coffee">
                    </div>
    ...
    <template x-teleport="body">
        ...
        >
            ...
            >
                <div class="flex items-center justify-between border-b border-coffee-latte pb-3">
                    <h3 class="font-extrabold text-coffee-dark">Ubah Nama Kategori</h3>
                </div>
    
                <form :action="`{{ url('/kategori/update') }}/${editCat.id_kategori}`" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="edit_kategori" class="block text-xs font-bold text-coffee-medium uppercase tracking-wider mb-2">Nama Kategori</label>
                        <input type="text" name="kategori" id="edit_kategori" x-model="editCat.kategori" required class="w-full px-4 py-2.5 rounded-xl border border-coffee-latte text-xs font-bold text-coffee-dark focus:outline-none focus:ring-2 focus:ring-coffee-light/50 bg-white">
                    </div>
